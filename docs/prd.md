...
## 15. 1차 개선사항 반영 (v1.1) — 2026-02-11

첫 캠페인(C001, C002) 실행 결과를 바탕으로 다음 개선사항을 반영하였다.

### 15.1 리서치 품질 향상

| 항목 | 변경 내용 |
|------|----------|
| **소셜미디어 활용** | YouTube, Instagram, TikTok 트렌드를 리서치에 통합. `social_trends.md` 생성 |
| **검색어 하드코딩 제거** | 고정 쿼리 패턴 삭제, 주제별 창의적 검색 원칙으로 교체 |
| **검색량 확대** | 10 WebSearch 제한 → 15-25 WebSearch, 5-10 WebFetch으로 유연 확장 |
| **출처 기록 강화** | 모든 데이터 포인트에 원본 URL 필수 기록, 출처 신뢰도 등급 표시 |

### 15.2 콘텐츠 연속성 & 시리즈

| 항목 | 변경 내용 |
|------|----------|
| **기존 콘텐츠 참조** | Writer가 기존 발행 콘텐츠를 읽고 자연스럽게 연결되는 내용 구성 |
| **시리즈 모드** | `series:true` 옵션으로 3-7편 단위 시리즈 콘텐츠 자동 설계 |
| **내부 링크** | Content Calendar에 Related 컬럼 추가, 이전 콘텐츠 참조 자동화 |

### 15.3 타겟팅 정밀화

| 항목 | 변경 내용 |
|------|----------|
| **타겟 옵션** | `country`, `age`, `gender` 옵션 추가 (모두 optional) |
| **미입력 시 동작** | 타겟팅 정보 없으면 리서치 기반 자율 타겟 설정 (기존 동작 유지) |
| **톤 맞춤** | 타겟팅 정보가 있으면 연령대/성별 맞춤 톤 자동 적용 |

### 15.4 이미지 검색 안정화

| 항목 | 변경 내용 |
|------|----------|
| **WebSearch 우선** | API 키 없이도 WebSearch로 이미지 검색 가능 (Priority 1으로 승격) |
| **한국어 검색** | 한국어 키워드로 한국 특화 이미지 검색 지원 |
| **기존 API** | Unsplash/Pexels API는 Priority 2로 유지 (API 키 있을 때만) |

### 15.5 발행 프로세스 효율화

| 항목 | 변경 내용 |
|------|----------|
| **Playbook 관리** | `workspace/config/playbooks/{platform}.md`에 검증된 발행 절차 관리 |
| **Naver Playbook** | C001 발행 경험 기반 SmartEditor insertText 방식 문서화 |
| **Playbook 자동 업데이트** | 발행 성공/실패 경험을 playbook에 즉시 반영 |
| **Headless 실행** | `.mcp.json`에 `--headless` 옵션 설정으로 백그라운드 발행 지원 |

### 15.6 글쓰기 품질 개선

| 항목 | 변경 내용 |
|------|----------|
| **간결성 원칙** | KO: 2,000-3,000자, EN: 1,000-1,500 words로 목표 분량 축소 |
| **가독성 강화** | 볼드 처리, 인용구, 구분선, 체크리스트 등 시각적 요소 규칙 추가 |
| **SEO 가독성** | SEO-Optimizer에 가독성 최적화 단계(볼드, 인용구, 구분선) 추가 |

### 15.7 할루시네이션 검사 강화

| 항목 | 변경 내용 |
|------|----------|
| **전수 추출** | 모든 사실적 주장(factual claim)을 식별하고 WebSearch로 검증 |
| **판정 체계** | VERIFIED / UNVERIFIABLE / FALSE / APPROXIMATION 4단계 판정 |
| **레퍼런스 URL** | 검증에 사용한 URL을 hallucination_report에 도메인 신뢰도와 함께 기록 |
| **검사 보고서** | verification_report.md에 주장별 검증 상세 테이블 생성 |
| **Writer 연동** | Writer가 글 작성 시 references 필드에 출처 URL 사전 수집 |

---

*MarketClaw PRD v1.1 - 2026.02*
